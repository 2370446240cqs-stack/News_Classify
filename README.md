...
(2)[sougou词向量下载](https://pan.baidu.com/s/1pUqyn7mnPcUmzxT64gGpSw)(本项目使用了其中SGNS格式的sougou新闻char词向量)
## 四、快速开始
(1)下载THUCNews数据集，将数据集中的内容放到THUCNews文件夹中，使用```process_data.py```进行数据预处理。使用devide.py将处理好的训练集进行划分
依次执行：
```
